	...
	namespace Graphics {
		...
		public:
			...
			Impl(ConstElement e, const char* path, IDirect3DDevice9* device) :
				...
				mName(0) {
				//한번 에러 상태로 둔다.성공하면 나중에 true로 한다
				mIsError = true;
				//이름이라든가 파일 이름이라든가를 빼다
			...
			Impl(const char* filename, IDirect3DDevice9* device) :
				...
				if (mExtension != EXT_UNKNOWN) {
					...
					mIsError = true; //모르는 파일 왔어.받지 않는다.
				}
			}
			...
			static void writeTga(const char* filename, int width, int height, int pitch, const unsigned* src, bool alpha) {
				...
				fileSize += 18; //헤더
				fileSize += (8 + footerStringLength + 1); //푸터
				...
				dst[0x1] = 0; // 팔레트 유무
				dst[0x2] = 2; //RGB24bit
				dst[0x3] = 0; // 팔레트 오프셋? (lower)
				dst[0x4] = 0; // 팔레트 오프셋? (upper)
				dst[0x5] = 0; // 팔레트 수? (lower)
				dst[0x6] = 0; // 팔레트 수? (upper)
				dst[0x7] = 0; // 팔레트 비트 심도?
				...
				dst[0x10] = pixelSize * 8;  // 색심도
				dst[0x11] = 0x20 + ((alpha) ? 8 : 0); // 위에서 아래 플래그와 3bit 눈은 알 수 없음
			...
			static void writeDds(const char* filename, int width, int height, int pitch, const unsigned* src, bool alpha) {
				...
				unsigned char* dst = reinterpret_cast<unsigned char*>(data);
				//DDS를 바이너리 에디터로 본 내용을 의미도 모르고 쓰고 있을 뿐
				dst[0x0] = 'D';
				dst[0x1] = 'D';
				dst[0x2] = 'S';
				dst[0x3] = ' ';
				dst[0x4] = 0x7c; //구조체 사이즈 124
				dst[0x8] = 0x7; //flags
				dst[0x9] = 0x10; //flags
				...
				dst[0x4c] = 0x20; //DDPIXELFORMAT의사이즈32
				dst[0x50] = 0x41; //flags
				dst[0x58] = 0x20; //32bit
				dst[0x5e] = 0xff; //Rmask
				dst[0x61] = 0xff; //Gmask
			...
			///그 자리에서 데이터를 파일에 써낸다.편리함수. pitch에는 대개 height*4가 들어가지만, 반드시 그렇지는 않다.
			...
			void createDxObject(int w, int h, bool createMipmaps) {
				...
				mDxDevice = 0; // 두 번 만들지는 않는다.여기서 끝난다.
				// 성공 후 대입
				mWidth = w;
				mHeight = h;
			}
			static int powerOfTwo(int a) {
				ASSERT(a < 0x40000000); //a가 0x40000000보다 크면 무한 루프가 된다.
				int r = 1;
				while (r < a) {
					r *= 2;
				}
			...
			void readTGA(const char* data, int size) {
				...
				const unsigned char* pSrc = 0; //파레트데이터
				src = reinterpret_cast<const unsigned char*>(data);
				pSrc = src + 18;
				src += 18 + paletteNumber * paletteDepth; //데이터 선두 취득
				//압축 대응
				Array< unsigned char > uncompressed;
				if (data[2] & 0x8) { //압축해서 하고 있어요.
				...
				if ((data[0x11] & 0x20) == 0) {
					...
					dy = -1;
					dSrc = -sw * pixelSize;
					src += sw * pixelSize * (sh - 1); //뒤에서 시작해야지
				}
				else {
					yBegin = 0;
					yEnd = sh;
				...
				if (pixelSize == 4) {
					for (int y = yBegin; y != yEnd; y += dy) {
						...
						for (int x = sw; x < pitch / 4; ++x) { //X여분을 까맣게
							dst[x] = 0;
						}
						dst += pitch;
						src += dSrc;
					...
					for (int y = yBegin; y != yEnd; y += dy) {
						for (int x = 0; x < sw; ++x) {
							...
							dst[x] |= src[x * 3 + 1] << 8;
							dst[x] |= src[x * 3 + 2] << 16;
							dst[x] |= 0xff000000; //A는255
						}
						for (int x = sw; x < pitch; ++x) { //X여분을 까맣게
							dst[x] = 0;
						}
						dst += pitch;
						src += dSrc;
				...
				if (pixelSize == 4) {
					...
					for (int y = 0; y < sh; ++y) {
						for (int x = 0; x < sw; ++x) {
							...
							dst[x] |= src[x * 4 + 2] << 16;
							dst[x] |= src[x * 4 + 3] << 24;
							dst[x] |= alphaOr; //XRGB 지원
						}
						for (int x = sw; x < pitch / 4; ++x) { //X여분을까맣게
							dst[x] = 0;
						}
						dst += pitch;
						src += (sw * pixelSize + 3) & (~3); //4바이트어라인
					}
				}
				else if (pixelSize == 3) {
					for (int y = 0; y < sh; ++y) {
			...
			void createMipmapChain(const unsigned* src, int srcPitch, int sw, int sh, int level) {
				...
				dstPitch /= 4;
				ASSERT(dstPitch >= dw); //여기가 작은 것은 있을 수 없다.
				// 일단 늦게라도 괜찮아.
				for (int y = 0; y < dh; ++y) {
					for (int x = 0; x < dw; ++x) {
						unsigned c0 = src[(2 * x)];
			...
			static int mipmapNumber(int w, int h) { //그 자리 계산. 둘 중 하나가 되면 거기서 끝.
				int levels = 0;
				while (w > 0 && h > 0) {
					w >>= 1;
					h >>= 1;
