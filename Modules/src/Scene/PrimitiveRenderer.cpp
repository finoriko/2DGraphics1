	...
	namespace Scene {
		...
		public:
			...
			Impl(
				...
				mBatchCapacity(batchCapacity + 1),//더미용으로+1
				mVertexPosition(0),
				mBatchPosition(0),
				mBatches(0),
				...
				mLockedVertexBuffer = mVertexBuffer.lock(); //초회잠금

				mBatches = NEW Batch[mBatchCapacity];
				mBatches[0].mPrimitiveNumber = 0x7fffffff; //0번은 더미 뱃지
				mPreviousFrameId = Manager().frameId() - 1; // 지나간 값으로
			}
			~Impl() {
			...
			Batch* addBatch() {
				...
				mBatches[mBatchPosition + 1] = mBatches[mBatchPosition]; //복사
				++mBatchPosition;
			...
			void addRectangle(
				...
				b->mPrimitiveNumber += 2; //삼각형 두 개인 점 주의
			...
			void draw() {
				...
				screenToClipTransform.setOrthogonalTransform(
					...
					static_cast<float>(height), 0.f, //아래, 위 순서로 아래는 height, 위로는 0이어야 한다는 점에 주의
					0.f, 1.f);
				//단위행렬
				...
				// 뱃지 하나씩 처리해 나간다.더미는 무시하기 때문에 1번부터
				for (int i = 1; i <= mBatchPosition; ++i) {
					...
					// 이것을 세트 여부는 프리미티브 유형에 따라
					if (b.mType == TYPE_RECTANGLE) {// 클립 공간화 행렬을 세트
					...
					else if (specialTransform || transformChanged) { //위 두 직후 또는 행렬이 바뀌어 있으면 세트
						m.setProjectionViewMatrix(transform);
					...
					//뱃치가 가지고 있는 텍스처 핸들을 모두 버림
					b.mTexture.release();
				}
				//정점버퍼 다음을 위해 잠금
				m.setVertexBuffer(0); //뺴고
