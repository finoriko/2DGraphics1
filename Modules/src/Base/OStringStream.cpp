...
namespace GameLib {
	...
	public:
		...
		void setPrecision(int a) {
			if (a > 17) {
				a = 17; //한계는 17. double의 10진 정밀도가 이 정도 된다.
			}
			else if (a < 1) {
				a = 1;
			}
		...
		void add(char a) {
			if ((a == '\n') || (a == '\t')) {
				mBuffer.add(a); //변환하지 않음(공백문자)
			}
			else if (a >= 0x20 && a <= 0x7e) {
				mBuffer.add(a); //변환하지 않음
			}
			else {
				mBuffer.add('%');
				char s[2];
				int c = toString16(s, a); //16진으로
				ASSERT(c <= 2);
				for (int i = 0; i < c; ++i) {
					mBuffer.add(s[i]);
				}
	...
	void OStringStream::get(string* a) const {
		if (mImpl->size() > 0) {
			Array< char > t;
			mImpl->get(&t);
			*a = string(&t[0], t.size() - 1); //ArraySize는 NULL 종단분 많으므로 뽑아주겠다
		}
	}
