...
namespace GameLib {
	namespace PseudoXml {
		...
		/// 태그를 텍스트에서 생성하는 클래스.
		/*!
		안의 문자열은 참조(Ref String)로, 자력으로 new해서 가진 것이 아니다.
		원래의 텍스트가 존재하는 동안만 사용하는 클래스라는 전제가 있다.
		또한 만든 Attribute . :: Impl*은 나중에 Element에게 넘겨주기 때문에 남는 것이 없다.
		*/
		class Tag {
		public:
			enum Type {
			...
			Tag(const char** p, const char* e, Document::Impl* document); //이것만은 cpp에 둔다.
			...
			~Tag() {
			// 만든 태그는 반드시 Element의 재료가 되어 있을 것이고,
			//m Attributes 는 give Attributes 로 넘겨버릴 것이다.
		...
		private:
			//XML적으로 특수한 의미를 가진 문자가 아닌지를 알아보는 함수
			static bool isNormalChar(char c, bool* wide) {
				...
				static const char table[0x100] = {
					...
					0, 2, 2, 2, 2, 2, 2, 2, //0x80-0x87 0x81-0x9f sjis 2바이트 영역
					...
					2, 2, 2, 2, 2, 0, 0, 0, //0xf8-0xff 0xe0-0xfc sjis 2바이트 영역
				};
				char t = table[u];
